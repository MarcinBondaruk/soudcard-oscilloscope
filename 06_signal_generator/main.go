...
var shapeNames = [...]string{"Sine", "Sawtooth", "Triangle", "Square"}
...
func squareWave(phase, dutyCycle float64) float64 {
	if phase < dutyCycle {
		return 1
	}
	return -1
...
func generateSample(shape WaveShape, phase, dutyCycle float64) float64 {
	switch shape {
	case Sine:
	...
	case Square:
		return squareWave(phase, dutyCycle)
	}
	return 0
}
...
func (g *Game) handleGeneratorInput() {
	if inpututil.IsKeyJustPressed(ebiten.KeyG) {
		g.mu.Lock()
		g.gen.enabled = !g.gen.enabled
		g.mu.Unlock()
	}
	...
	if inpututil.IsKeyJustPressed(ebiten.KeyE) {
		g.mu.Lock()
		g.gen.frequency *= 1.1
		if g.gen.frequency > 20000 {
			g.gen.frequency = 20000
		}
		g.mu.Unlock()
	}
	...
	if inpututil.IsKeyJustPressed(ebiten.KeyA) {
		g.mu.Lock()
		g.gen.amplitude += 0.05
		if g.gen.amplitude > 1.0 {
			g.gen.amplitude = 1.0
		}
		g.mu.Unlock()
	}
	...
	if inpututil.IsKeyJustPressed(ebiten.KeyD) {
		...
		if ebiten.IsKeyPressed(ebiten.KeyShift) {
			g.gen.dutyCycle -= 0.05
			if g.gen.dutyCycle < 0.05 {
				g.gen.dutyCycle = 0.05
			}
		} else {
			g.gen.dutyCycle += 0.05
			if g.gen.dutyCycle > 0.95 {
				g.gen.dutyCycle = 0.95
			}
		}
		g.mu.Unlock()
	}
...
func (g *Game) drawGeneratorStatus() {
	g.mu.Lock()
	enabled := g.gen.enabled
	shape := g.gen.shape
	freq := g.gen.frequency
	amp := g.gen.amplitude
	duty := g.gen.dutyCycle
	g.mu.Unlock()
	...
	ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("Generator: %s  [G toggle]", status), x, y)

	if enabled {
		ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("Shape: %s  [1-4]", shapeNames[shape]), x, y+20)
		ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("Freq:  %.1f Hz  [E/Q]", freq), x, y+40)
		ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("Amp:   %.2f  [A/Z]", amp), x, y+60)
		if shape == Square {
			ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("Duty:  %.0f%%  [D/Shift+D]", duty*100), x, y+80)
		}
	}
}
...
func (g *Game) drawStatus() {
	...
	case Stopped:
		if len(g.samples) == 0 {
			ebitenutil.DebugPrintAt(g.canvas, "[Space to start recording]  [G toggle generator]", 10, y)
		} else {
			dur := float64(len(g.samples)) / float64(sampleRate)
			ebitenutil.DebugPrintAt(g.canvas, fmt.Sprintf("STOPPED  %.1fs  zoom:%dx  [Space]record  [M]measurements  [G]generator", dur, g.viewport.zoom), 10, y)
		}
	}
}
...
func InitAudio(g *Game) error {
	...
	// Input stream
	inputBuffer := make([]float32, framesPerRead)
	inputStream, err := portaudio.OpenDefaultStream(1, 0, float64(sampleRate), framesPerRead, inputBuffer)
	if err != nil {
		terminateErr := portaudio.Terminate()
		if terminateErr != nil {
			slog.Error("portaudio terminate failed", "err", terminateErr)
		}
		return errors.Join(errors.New("failed to open input stream"), err)
	}

	err = inputStream.Start()
	if err != nil {
		closeErr := inputStream.Close()
		if closeErr != nil {
			slog.Error("input stream close failed", "err", closeErr)
		}
		terminateErr := portaudio.Terminate()
		if terminateErr != nil {
			slog.Error("portaudio terminate failed", "err", terminateErr)
		}
		return errors.Join(errors.New("failed to start input stream"), err)
	}
	g.inputStream = inputStream

	// Output stream
	outputBuffer := make([]float32, framesPerRead)
	outputStream, err := portaudio.OpenDefaultStream(0, 1, float64(sampleRate), framesPerRead, outputBuffer)
	if err != nil {
		stopErr := inputStream.Stop()
		if stopErr != nil {
			slog.Error("input stream stop failed", "err", stopErr)
		}
		closeErr := inputStream.Close()
		if closeErr != nil {
			slog.Error("input stream close failed", "err", closeErr)
		...
		return errors.Join(errors.New("failed to open output stream"), err)
	}

	err = outputStream.Start()
	if err != nil {
		closeErr := outputStream.Close()
		if closeErr != nil {
			slog.Error("output stream close failed", "err", closeErr)
		}
		stopErr := inputStream.Stop()
		if stopErr != nil {
			slog.Error("input stream stop failed", "err", stopErr)
		...
		return errors.Join(errors.New("failed to start output stream"), err)
	}
	...
	go func() {
		...
		for g.running {
			...
			if err != nil {
				slog.Error("input stream read error", "err", err)
				continue
			}

			g.mu.Lock()
			if g.state == Recording {
				g.samples = append(g.samples, inputBuffer...)
			}
			g.mu.Unlock()
		}
	}()
	...
	go func() {
		...
		for g.running {
			...
			if !enabled {
				...
				for i := range outputBuffer {
					sample := generateSample(shape, phase, duty)
					outputBuffer[i] = float32(sample * amp)
					phase += step
					if phase >= 1.0 {
						phase -= 1.0
					}
			...
			if err != nil {
				slog.Error("output stream write error", "err", err)
			}
		}
	}()
...
func main() {
	...
	ebiten.SetWindowTitle("signal generator")
	...
	if stopErr := game.outputStream.Stop(); stopErr != nil {
		slog.Error("output stream stop failed", "err", stopErr)
	}
	if closeErr := game.outputStream.Close(); closeErr != nil {
		slog.Error("output stream close failed", "err", closeErr)
	}
	if stopErr := game.inputStream.Stop(); stopErr != nil {
		slog.Error("input stream stop failed", "err", stopErr)
	}
	if closeErr := game.inputStream.Close(); closeErr != nil {
		slog.Error("input stream close failed", "err", closeErr)
	}
